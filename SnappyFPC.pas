...
procedure _assert(__cond, __file: Pchar; __line: Integer); cdecl;  alias : '_assert';
begin
  raise Exception.CreateFmt('Assertion failed: %s, file %s, line %d', [__cond, __file, __line]);
end;

{$IFDEF WIN64}
procedure free(P: pointer); cdecl; alias : 'free';
begin
  FreeMem(P);
end;

function malloc(size: Integer): pointer; cdecl; alias : 'malloc';
begin
  GetMem(result, size);
end;

function memcmp(s1, s2: pointer; n: Integer): Integer; cdecl; alias : 'memcmp';
begin
  result := StrLComp(PChar(s1), PChar(s2), n);
end;

procedure memcpy(dest, source: pointer; count: Integer); cdecl; alias : 'memcpy';
begin
  Move(source^, dest^, count);
end;

function memmove(dest, src: pointer; n: Cardinal): pointer; cdecl; alias : 'memmove';
begin
  Move(src^, dest^, n);
  result := dest;
end;

procedure memset(P: pointer; B: Integer; count: Integer); cdecl; alias : 'memset';
begin
  FillChar(P^, count, B);
end;
{$ENDIF}
{$IFDEF WIN32}
procedure _free(P: pointer); alias : '_free';
begin
  FreeMem(P);
end;

function _malloc(size: Integer): pointer; alias : '_malloc';
begin
  GetMem(result, size);
end;

function _memcmp(s1, s2: pointer; n: Integer): Integer; alias : '_memcmp';
begin
  result := StrLComp(PChar(s1), PChar(s2), n);
end;

procedure _memcpy(dest, source: pointer; count: Integer); alias : '_memcpy';
begin
  Move(source^, dest^, count);
end;

function _memmove(dest, src: pointer; n: Cardinal): pointer; alias : '_memmove';
begin
  Move(src^, dest^, n);
  result := dest;
end;

procedure _memset(P: pointer; B: Integer; count: Integer); alias : '_memset';
begin
  FillChar(P^, count, B);
end;
{$ENDIF}
